...
- **Activity** → append to `engagement/activity.md` at milestones:
  `### [YYYY-MM-DD HH:MM:SS] linux-file-path-abuse → <hostname>` with actions and results.
...
1. **On-screen**: Print `[linux-file-path-abuse] Activated → <target>` so the operator
   sees which skill is running.
2. **activity.md**: Append:
   ```
   ### [YYYY-MM-DD HH:MM:SS] linux-file-path-abuse → <target>
