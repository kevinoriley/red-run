...
- **Activity** → append to `engagement/activity.md` at milestones (race confirmed,
  ...
  `### [YYYY-MM-DD HH:MM:SS] race-condition → <target>` with bullet points of actions and results.
...
1. **On-screen**: Print `[race-condition] Activated → <target>` so the operator
   sees which skill is running.
2. **activity.md**: Append:
   ```
   ### [YYYY-MM-DD HH:MM:SS] race-condition → <target>
