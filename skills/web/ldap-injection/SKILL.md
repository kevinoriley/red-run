...
- **Activity** → append to `engagement/activity.md` at milestones (auth bypass
  ...
  `### [YYYY-MM-DD HH:MM:SS] ldap-injection → <target>` with bullet points of actions/results.
...
1. **On-screen**: Print `[ldap-injection] Activated → <target>` so the operator
   sees which skill is running.
2. **activity.md**: Append:
   ```
   ### [YYYY-MM-DD HH:MM:SS] ldap-injection → <target>
