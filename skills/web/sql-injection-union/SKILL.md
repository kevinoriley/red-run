...
- **Activity** → append to `engagement/activity.md` at milestones (test confirmed,
  ...
  `### [YYYY-MM-DD HH:MM:SS] sql-injection-union → <target>` with bullet points of actions/results.
...
1. **On-screen**: Print `[sql-injection-union] Activated → <target>` so the operator
   sees which skill is running.
2. **activity.md**: Append:
   ```
   ### [YYYY-MM-DD HH:MM:SS] sql-injection-union → <target>
