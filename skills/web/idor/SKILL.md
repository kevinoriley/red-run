...
- **Activity** → append to `engagement/activity.md` at milestones (access control
  ...
  `### [YYYY-MM-DD HH:MM:SS] idor → <target>` with bullet points of actions/results.
...
1. **On-screen**: Print `[idor] Activated → <target>` so the operator
   sees which skill is running.
2. **activity.md**: Append:
   ```
   ### [YYYY-MM-DD HH:MM:SS] idor → <target>
