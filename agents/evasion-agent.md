...
**Sliver implant evasion:**
When building AV-safe payloads for Sliver deployment, consider:
- `sliver_generate(evasion=True)` enables built-in Sliver evasion
- Custom loaders can wrap Sliver shellcode
- Sliver's own stager/stageless options affect detection rates
